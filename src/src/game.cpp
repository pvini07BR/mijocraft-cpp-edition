...
void Game::Init()
{
    ...
    player = new ActivePlayer(glm::vec4(1.0f, 0.0f, 0.0f, 1.0f), world);
    Renderer = new SpriteRenderer(myShader);
    textRender = new TextRenderer(this->Width, this->Height);
    textRender->Load("assets/fonts/nokiafc22.ttf", 24);
    // load textures
...
{   
    ...
    {
        Renderer->DrawSprite(wallRend->texture, wallRend->position, glm::vec2(wallRend->scale, wallRend->scale), 0.0f, glm::vec3(0.5f, 0.5f, 0.5f));
    }

    std::vector<Block>::iterator blockRend;
    for (blockRend = blocks.begin(); blockRend != blocks.end(); ++blockRend)
    {
        Renderer->DrawSprite(blockRend->texture, blockRend->position, glm::vec2(blockRend->scale, blockRend->scale), 0.0f, glm::vec3(1.0f, 1.0f, 1.0f));
    }
    ...
    textRender->RenderText("teste", (float)Width - 24.0f * 5, (float)Height - 24.0f, 1.0f);
    textRender->TextShader.SetMatrix4("projection", glm::ortho(0.0f, (float)Width, (float)Height, 0.0f), true);
}
...
void Game::ImGuiRender()
{
    ...
    {
        ImVec4 playerColor = ImVec4(player->color.x, player->color.y, player->color.z, player->color.w);
        if (ImGui::ColorPicker4("Color", (float*)&playerColor))
        {
            player->color = glm::vec4(playerColor.x, playerColor.y, playerColor.z, playerColor.w);
        }
...
void BlockManager::PlaceBlock(unsigned int type, int posX, int posY, PLACE_MODES placMod)
{
    ...
    {
        Block *b = new Block(blockTextures[type], posX, posY, type, world);
        if(placMod == BLOCK)
        {
            blocks.push_back(*b);
        } else if(placMod == WALL){
            world->DestroyBody(b->groundBody);
            walls.push_back(*b);
        }
    }
}
